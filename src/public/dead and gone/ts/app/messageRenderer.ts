...
const proxy: string = `${location.protocol}//${location.host}/api/proxy/image?url=`

export async function renderMessage(message: Syrenity.Message, skipUser: boolean = false) {
  // Fetch the author
  ...
  if (!skipUser) {
    ...
    pfp.onclick = () => {
      ...
      showUserDiv(messageAuthor as Syrenity.User, pfp);
    }
  ...
  if (skipUser) messageContent.classList.add("message-content-short");
...
async function scanForImages(message: Syrenity.Message): Promise<HTMLImageElement[]> {
...
function generateUsername(user: Syrenity.User, data: { isOwner: boolean }) {
  let string = `${user.username}`;

  // Check if bot
  if (user.isBot) string += `<label class="message-tag">Bot</label>`;
  if (data.isOwner) string += `<label class="message-tag">Owner</label>`;
