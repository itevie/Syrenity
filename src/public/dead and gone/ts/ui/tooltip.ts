...
interface TooltipDetails {
  /**
   * The element to attach the tooltip to and listen for clicks
   */
  ...
  /**
   * In which direction the tooltip should flyout once it's shown
   */
  flyout: 'left' | 'right' | 'down' | 'top';

  /**
   * Whether it should follow the mouse X instead of the element's
   */
  followMouseX?: boolean;

  /**
   * How long the mouse has to be hovered on the element for it to show
   */
  hoverFor?: number;
  
  /**
   * Whether or not the tooltip should show exactly where the mouse is
   */
  exactMouse?: boolean;
}

const tooltips = new Map<string, TooltipDetails>();
let activeTooltip: HTMLElement = null;

/**
 * Registers a tooltip listener for an element
 * @param details The settings for the tooltip
 */
export function registerTooltip(details: TooltipDetails) {
  // Register tooltip
 ...
 */
function showTooltip(details: TooltipDetails, event: MouseEvent) {
  resetTooltip();
  const tooltipContainer = document.getElementById("tooltip-container");
  tooltipContainer.style.display = "block";

  if (!details.flyout) details.flyout = "right";
