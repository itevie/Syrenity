...
export default function MessageC({message}: {message: Message}) {
  ...
  return (
    <Row util={["no-shrink"]} style={{gap: "10px"}} onContextMenu={e => showMsgContextMenu(e)}>
      <Icon size="48px" src={File.check(users[message.authorId]?.avatar)} fallback="/public/images/logos/no_shape_logo.png" />
      <Column util={["flex-grow"]} style={{gap:"5px"}}>
        <Row util={["align-center"]} style={{gap: "10px"}}>
          <b>{users[message.authorId]?.username ?? `Loading... (ID ${message.authorId})`}</b>
