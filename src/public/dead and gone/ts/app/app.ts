...
import { loadAll as loadAllTooltips, registerTooltip } from '../ui/tooltip.js';
...
export async function showUserDiv(user: Syrenity.User, element: HTMLElement) {
  setTimeout(() => {
    const container = document.getElementById("user-profile-container");
    
    (document.getElementById("user-profile-avatar") as HTMLImageElement).src = user.avatar;
    ...
    document.getElementById("user-profile-avatar").onclick = () => {
      enlargeImage((document.getElementById("user-profile-avatar") as HTMLImageElement).src);
    }
    ...
    const fix = () => {
      cancelThief = true;
      let color = general_new.avatar.setNoAvatarIcon(user.id, document.getElementById("user-profile-avatar") as HTMLImageElement);
    ...
    if ((element as HTMLImageElement).complete && cancelThief == false) {
      updateColor(colorThief.getColor(element));
    } else if (cancelThief == false) {
      (element as HTMLImageElement).addEventListener('load', function() {
        if (cancelThief) return;
...
function addServer() {
  ...
  general_new.modals.showModal("add_server", {
    handleIDs: (keys) => {
      const inviteCodeElement = keys["invite-code"] as HTMLInputElement;
      ...
      keys["join"].onclick = async () => {
        try {
          ...
          else general_new.alerts.error(`This is a placeholder message: ${JSON.stringify(err.rawData || err.rawResponse || { unknown: "unknown" })}`, "Failed to join server");
        }
      }
  ...
  general_new.modals.showModal("create_server", {
    handleIDs: (keys) => {
      ...
      keys["create"].onclick = async () => {
        // Gather data
        const name = (keys["server-name"] as HTMLInputElement).value;
        const avatar = (keys["avatar"] as HTMLImageElement).src.split(",")[1];
        const uploadedAvatar = (keys["avatar"] as HTMLImageElement).hasAttribute("data-modal-upload-success");
...
export function parseUrl(): {channel: number | null, guild: number | "@me" | null} {
