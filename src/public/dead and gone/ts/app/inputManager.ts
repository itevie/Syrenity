...
const managers = new Map<number, InputManager>();
let activeManager: InputManager | null = null;
const input = document.getElementById("input-input") as HTMLInputElement;
...
interface TypingDetails {
  user: Syrenity.User;
  channel: Syrenity.BaseChannel;
  startedAt: number;
}

export class InputManager {
  public draft: string = "";
  public channel: Syrenity.Channel;
  public typing: Map<number, TypingDetails> = new Map<number, TypingDetails>();
  public lastSentTyping: number = 0;

  constructor(channel: Syrenity.Channel) {
  ...
  public renderTyping() {
    ...
    this.typing.forEach((value, key) => {
      ...
      if (typing.length === 3)
        others++;
      else typing.push(value.user.username);
    });
    ...
    for (let i in typing) {
      ...
      if (typing.length == 1) continue;
      else if (parseInt(i) == typing.length - 2 && !others) text += " and ";
      else if (parseInt(i) != typing.length - 1) text += ", ";
    }
  ...
  public static async handleTyping(event: {user: Syrenity.BaseUser, channel: Syrenity.BaseChannel, guild: Syrenity.BaseGuild}) {
  ...
  public static async removeTyping(forUser: number, inChannel: number) {
...
export default {
  loadManager: (channel: Syrenity.Channel) => {
